...
async function pollTask(task_id: string) {
  ...
  if (!task.response && !task.result) {
    return NextResponse.json(
      {
        success: false,
        status: 'completed',
        response: { status: 'error', result: {}, message: 'Agent completed but returned no response data' },
        error: 'Agent completed but returned no response data',
      },
      { status: 500 }
    )
  }
  // Get the raw response - could be string or object
  const taskResponse = task.response ?? task.result
  let moduleOutputs: ModuleOutputs | undefined
  let agentResponseRaw: any = taskResponse

  // If response is a string, try to parse it
  if (typeof taskResponse === 'string') {
    try {
      agentResponseRaw = JSON.parse(taskResponse)
    } catch {
      // Keep as string - parseLLMJson will handle it
    }
  }

  // Check for Lyzr envelope: { response: ..., module_outputs: ... }
  if (agentResponseRaw && typeof agentResponseRaw === 'object' && !Array.isArray(agentResponseRaw)) {
    if ('module_outputs' in agentResponseRaw) {
      moduleOutputs = agentResponseRaw.module_outputs
    }
    if ('response' in agentResponseRaw && agentResponseRaw.response != null) {
      const inner = agentResponseRaw.response
      // Unwrap inner response - could be string or object
      if (typeof inner === 'string') {
        try {
          agentResponseRaw = JSON.parse(inner)
        } catch {
          agentResponseRaw = inner
        }
      } else {
        agentResponseRaw = inner
      }
  ...
  // Use parseLLMJson for robust parsing of agent output
  const parsed = parseLLMJson(agentResponseRaw)

  // If parseLLMJson returned a failure sentinel, fall back to the raw data
  const toNormalize =
    parsed && typeof parsed === 'object' && parsed.success === false && parsed.data === null
      ? (typeof agentResponseRaw === 'string' ? { text: agentResponseRaw } : agentResponseRaw)
      : parsed

  const normalized = normalizeResponse(toNormalize)

  // Reflect agent-level errors in the top-level success field
  const isAgentError = normalized.status === 'error'

  const rawText = typeof taskResponse === 'string' ? taskResponse : JSON.stringify(taskResponse)

  return NextResponse.json({
    success: !isAgentError,
    status: 'completed',
    response: normalized,
    module_outputs: moduleOutputs,
    timestamp: new Date().toISOString(),
    raw_response: rawText,
    ...(isAgentError ? { error: normalized.message || 'Agent returned an error' } : {}),
  })
}
