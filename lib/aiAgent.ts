...
export async function callAIAgent(
  ...
  try {
    ...
    try {
      ...
      return {
        success: false,
        response: { status: 'error', result: {}, message: `Server returned invalid response (HTTP ${submitRes.status})` },
        error: `Server returned invalid response (HTTP ${submitRes.status})`,
      }
    }

    if (!submitRes.ok && !submitData.task_id) {
      const errMsg = submitData?.error || submitData?.response?.message || `Server error (HTTP ${submitRes.status})`
      return {
        success: false,
        response: { status: 'error', result: {}, message: errMsg },
        error: errMsg,
      }
    }
